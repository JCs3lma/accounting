...
<ul class="flex flex-col gap-2">
    ...
    <x-list-item class="pl-0 {{$currentActiveMenu == 'shops' ? 'group stroke-white group-hover:stroke-gray-500' : ''}}">
        <x-slot:icon class="iconMenu"><x-user-setting-icon class="{{$currentActiveMenu == 'price' ? 'fill-white group-hover:fill-gray-700' : ''}}"/></x-slot:icon>
    ...
    <x-list-item class="pl-0 {{$currentActiveMenu == 'suppliers' ? 'group stroke-white group-hover:stroke-gray-500' : ''}}">
        ...
        <x-slot:link href="{{ route('suppliers.index') }}" class="textMenu {{request()->routeIs('suppliers.index') ? 'bg-blue-500 text-white hover:text-gray-800' : ''}}">Supplier's Products</x-slot:link>
    </x-list-item>
</ul>